...
Mancala botVsBot(int heu1, int heu2, int depth,bool print=false){
    Mancala mancala;
    ...
    while(true){
        ...
        if(mancala.current_turn() == 1){
            mancala.play_turn(player1.suggest_next_move(mancala, depth));
        }else if(mancala.current_turn() == 2){
            mancala.play_turn(player2.suggest_next_move(mancala, depth));
        }else{
            if(mancala.win() != -1){
                break;
            }
...
void generateReport(int depth,ofstream &fout){
	int p1win=0,p2win=0,draw=0;
    for(int i = 1; i <= 4; i++){
        for(int j = 1; j <=4 ; j++){
			p1win=0,p2win=0,draw=0;
            for(int k=1;k<=100;k++){
				Mancala mancala = botVsBot(i, j, depth);
				if(mancala.get_p1_score() > mancala.get_p2_score()){
					p1win++;
				}else if(mancala.get_p1_score() < mancala.get_p2_score()){
					p2win++;
				}else{
					draw++;
				}	
			}
			fout<<i<<" "<<j<<" "<<depth<<" "<<w1<<" "<<w2<<" "<<w3<<" "<<w4<<" "<<p1win<<" "<<p2win<<" "<<draw<<endl; 
        }   
    }
}
